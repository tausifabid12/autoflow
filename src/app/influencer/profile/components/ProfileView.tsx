          ...
          <div className="flex flex-col md:flex-row gap-6 items-start md:items-center">
            ...
            <div className="flex-1 space-y-3">
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
                <div>
                  <h1 className="text-3xl font-bold bg-accent bg-clip-text text-transparent">
                    {user?.firstName} {user?.lastName}
                  </h1>
                  <p className="text-lg text-muted-foreground">@{user?.username}</p>
                </div>
                <div className='flex gap-2'>
                  <Link href={`/influencer-public-profile?influencer=${user?.slug}`}>
                   <Button
                  onClick={onEditClick}
        ...
        <Card className="p-6 shadow-lg border-2 hover:shadow-xl transition-shadow duration-300">
          ...
          <div className="grid md:grid-cols-2 gap-4">
            <div className="flex items-start gap-3 p-4 bg-card  rounded-xl border border-border hover:shadow-md transition-shadow">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm text-muted-foreground font-medium">Bank Account Holder Name</p>
                <p className="font-semibold truncate">{user?.bank_account_holder_name}</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3 p-4 bg-card  rounded-xl border border-border hover:shadow-md transition-shadow">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm text-muted-foreground font-medium">Bank Account Number</p>
                <p className="font-semibold">{user?.bank_account_number}</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3 p-4 bg-card  rounded-xl border border-border hover:shadow-md transition-shadow">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="font-semibold">{user?.bank_ifsc_code}</p>
              </div>
            </div>
            ...
            <div className="flex items-start gap-3 p-4 bg-card  rounded-xl border border-border hover:shadow-md transition-shadow">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm text-muted-foreground font-medium">Bank Branch</p>
                <p className="font-semibold">{user?.bank_branch}</p>
              </div>
            </div>
          ...
          <Card className="p-6 shadow-lg border-2 hover:shadow-xl transition-shadow duration-300">
            ...
            <div className="flex flex-wrap gap-2">
              {user?.industry.map((item, index) => (
                <Badge
                  key={index}
                  className="bg-gradient-to-r from-purple-500 to-pink-500  px-4 py-2 text-sm shadow-md hover:shadow-lg transition-all duration-200 hover:scale-105"
                >
                  {item}
                </Badge>
              ))}
